...
describe('Test Card Detail', () => {
	...
	test('Test card detail members', () => {
		...
		const cardDetail = render(
			<CardDetail entity={nonFollowingEntity} members={members} />
		);
		...
		const cardWithoutMembers = render(
			<CardDetail entity={nonFollowingEntity} />
		);
		expect(
			cardWithoutMembers.container.querySelector('.members')
		).toBeFalsy();
		expect(
			cardWithoutMembers.container.querySelector('.member-list')
		).toBeFalsy();
	...
	test('Test card detail flyout options', async () => {
		...
		const flyoutOption = (
			<React.Fragment>
				<div className="mock-flyout-option" key="mock option">
					{mockOptionText}
				</div>
			</React.Fragment>
		);
		...
		const cardDetail = render(
			<CardDetail
				entity={nonFollowingEntity}
				flyoutOptions={flyoutOption}
			/>
		);
		expect(
			cardDetail.baseElement.querySelector('.mock-flyout-option')
		).toBeFalsy();
	...
	test('Test card detail name and rename mode', async () => {
		...
		expect(
			cardDetail.container.querySelector('.display-name')
		).toBeTruthy();
		expect(await cardDetail.findByText(alias)).toBeTruthy();
		expect(
			cardDetail.container.querySelector('.editable-text-field')
		).toBeFalsy();
		...
		const modifiableCard = render(
			<CardDetail entity={modifiableEntity} renameMode={false} />
		);
		expect(
			modifiableCard.container.querySelector('.editable-text-field')
		).toBeTruthy();
		expect(
			modifiableCard.container.querySelector('.editable-text-field input')
		).toBeFalsy();

		const modifiableCardWithRename = render(
			<CardDetail entity={modifiableEntity} renameMode={true} />
		);
