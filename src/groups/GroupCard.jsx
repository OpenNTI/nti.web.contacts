...
const EditLinkWrapper = ({ entity, ...props }) => (
	<LinkTo.Path to={editorPath(entity)} draggable={false} {...props} />
);
				...
				<CardDetail
					...
					flyoutOptions={
						<React.Fragment>
							{canEdit && (
								<LinkTo.Path
									to={editorPath(entity)}
									className="group-action-flyout-option"
								>
									{t('editGroupText')}
								</LinkTo.Path>
							)}
							...
							{entity.hasLink(
								'default-trivial-invitation-code'
							) && (
								<LinkTo.Path
									to={invitePath}
									className="group-action-flyout-option"
							...
							{!entity.friends && (
								<div
									className="group-action-flyout-option-delete"
...
export default decorate(GroupCard, [GroupListStore.connect()]);
