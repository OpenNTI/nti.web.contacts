...
const t = scoped(
	'nti-web-contacts.sharing-lists.SharingListManagePeopleModal',
	{
...
class SharingListManagePeopleModal extends React.Component {
	...
	static getDerivedStateFromProps({ entityId, store }, state) {
		...
		const item = items.find(
			x => x.getID() === decodeURIComponent(entityId)
		);
	...
	addContactToList = newContact => {
		...
		if (
			(existingContacts || []).find(i => i.getID() !== newContact.getID())
		) {
	...
	renderControls = () => {
		const buttons = [{ label: 'Done', onClick: this.onFinishedEditing }];
