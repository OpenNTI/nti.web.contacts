...
class SharingListCreateModal extends React.Component {
	...
	removeContactFromList = contactToRemove => {
		...
		const newContacts = contacts.filter(
			e => e.getID() !== contactToRemove.getID()
		);
	...
	renderControls = () => {
		...
		const disableCreateClass = (this.disableCreate = sharingListName.trim()
			? ''
			: 'disabled');
		...
		const buttons = [
			...
			{
				label: 'Create',
				onClick: this.onCreateSharingList,
				className: disableCreateClass,
			},
		];
	...
	renderContent = () => {
		...
		return (
			<div className="sharing-list-action-modal-content">
				...
				<Input.Text
					placeholder="Name"
					value={this.state.sharingListName}
					onChange={this.updateSharingListName}
					maxLength="80"
				/>
				<div className="sharing-list-action-modal-content sub-header">
		...
		return (
			<Prompt.Dialog onBeforeDismiss={this.onDismiss}>
				<div className="sharing-list-action-modal">
					<Panels.Header
						className="sharing-list-action-modal-header"
