...
const t = scoped('nti-web-contacts.commons.Members', {
...
export default function Members({ members, displayLabel = true }) {
	...
	function renderMemberList() {
		return members
			...
			.map(i => <Member entity={i} key={i.Username} />);
	}

	let listOfMembers = renderMemberList();
	if (members.length > MAX_MEMBERS_TO_DISPLAY) {
		listOfMembers.unshift(
			<MemberOverflowPlaceholder
				overflowCount={members.length - MAX_MEMBERS_TO_DISPLAY}
				key="overflowPlaceholder"
			/>
		);
	}
		...
		<Flyout.Triggered
			...
			trigger={
				<div className="members">
					{displayLabel && (
						<div className="members-label">{t('membersLabel')}</div>
					)}
					<div className="member-list">{listOfMembers.reverse()}</div>
				</div>
			}
		>
			{members.slice(0, MAX_MEMBERS_IN_FLYOUT).map(x => (
				<div className="members-hover" key={x.Username}>
					{x.alias.toUpperCase()}
				</div>
			))}
